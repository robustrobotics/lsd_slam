...
#include "stats_tracker/assert.h"
...
inline void getSystemCPULoad(const SystemCPUMeasurement& prev,
  ...
  STATS_TRACKER_ASSERT(cpu_sys_total >= cpu_sys_total_prev);
...
inline void getProcessCPULoad(const ProcessCPUMeasurement& prev,
  ...
  STATS_TRACKER_ASSERT(cpu_pid_total >= cpu_pid_total_prev);

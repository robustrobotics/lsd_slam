...
#ifdef STATS_TRACKER_NO_ASSERT
#define STATS_TRACKER_ASSERT(x)
#else
#include <cxxabi.h>
#include <unistd.h>
...
inline void assert_fail(const char *condition, const char *function,
  ...
  fprintf(stderr, "STATS_TRACKER_ASSERT failed: %s in function %s at %s: %i\n",
...
#define STATS_TRACKER_ASSERT(condition) \
  do { \
    if (!(condition)) \
      stats_tracker::assert_fail(#condition, __PRETTY_FUNCTION__, __FILE__, __LINE__); \
  } while (false)
